-- Drop and recreate function with session link
...
AS $function$
  SELECT
    ...
    CASE 
      WHEN p.link_type = 'recurring_meet' AND p.recurring_meet_code IS NOT NULL 
        THEN 'https://meet.google.com/' || p.recurring_meet_code
      WHEN p.link_type = 'external' AND p.external_session_link IS NOT NULL 
        THEN p.external_session_link
      WHEN p.session_link IS NOT NULL 
        THEN p.session_link
      ELSE NULL
    END as session_link
  FROM sessions s
