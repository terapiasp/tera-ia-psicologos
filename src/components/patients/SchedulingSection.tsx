...
export const SchedulingSection = ({
  ...
  const [schedulingMode, setSchedulingMode] = useState<'single' | 'recurring'>(value ? 'recurring' : 'single');
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <Button type="button" variant={schedulingMode === 'single' ? 'default' : 'outline'} onClick={() => handleModeChange('single')} className="flex-1 h-12">
              <Calendar className="h-4 w-4 mr-2" />
              Sessão Única
            </Button>
            <Button type="button" variant={schedulingMode === 'recurring' ? 'default' : 'outline'} onClick={() => handleModeChange('recurring')} className="flex-1 h-12">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <Input type="date" value={currentDate} onChange={e => handleDateChange(e.target.value)} className="h-12" />
          </div>
          <div>
            <Label className="text-sm font-medium mb-2 block">
            ...
            <TimeInput value={currentTime} onChange={handleTimeChange} className="h-12" />
          </div>
        </div>

        {/* Recurring Options - Only when recurring mode is selected */}
        {schedulingMode === 'recurring' && <div className="space-y-4 p-4 bg-muted/30 rounded-lg border">
            {/* Frequency Selector */}
            <div>
              <Label className="text-sm font-medium mb-2 block">Frequência</Label>
              <Select value={value?.frequency || 'weekly'} onValueChange={handleFrequencyChange}>
            ...
            {value && <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {DAYS_OF_WEEK.map(day => <Button key={day.value} type="button" variant={value.daysOfWeek?.includes(day.value) ? "default" : "outline"} size="sm" onClick={() => handleDayToggle(day.value)} className="h-8 px-3 text-xs">
                      {day.label}
                    </Button>)}
                </div>
                {(value.frequency === 'weekly' || value.frequency === 'biweekly') && <p className="text-xs text-muted-foreground mt-1">
